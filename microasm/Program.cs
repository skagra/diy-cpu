...
namespace microasm
{
   public class MS
   {
      ...
      private static readonly Dictionary<string, byte[]> _flagSymbols = new();
      private static readonly Dictionary<string, byte[]> _ucopsSymbols = new();
      private static readonly Dictionary<string, byte[]> _labelSymbols = new();
      private static readonly Dictionary<string, byte[]> _codeSymbols = new();
      ...
      private static string CreateFlagsHeader()
      {
         var maxSymbolLen=_flagSymbols.Keys.Max(k => k.Length);
         var numSymbols = _flagSymbols.Keys.Count;

         var canvas = new VertPrinter(maxSymbolLen, numSymbols);

         for (int index=0; index<numSymbols; index++)
         {
            canvas.Print(_flagSymbols.ElementAt(index).Key, index);
         }
      ...
      private static byte[] ResolveSymbol(string symbol, string line, int lineNumber)
      {
         byte[] result;
      ...
      private static void DumpSymbols(Dictionary<string, byte[]> symbols)
      {
         symbols.Keys.OrderBy(k => k).ToList().ForEach(k =>
         {
            ...
            Console.Write($" {value[3]:X2} {value[2]:X2} {value[1]:X2} {value[0]:X2}");
            Console.WriteLine();
         });
      }
         ...
         { // pass 1
            if (!line.StartsWith(LABEL))
            {
               ...
               else
               {
                  ...
                  {
                     opLine += ByteToBitString(ROM[_romAddress * 4 + offset]).Replace('0', '.');
                     if (offset != 0)
                     {
                        opLine += " ";
                     }
                  }
                  opLine+=($" {value[3]:X2} {value[2]:X2} {value[1]:X2} {value[0]:X2}");
      ...
      public static void Main(string[] args)
      {
         ...
         try
         {
            ...
            var flagsHeader = CreateFlagsHeader();

            Console.WriteLine("Flags Symbol Table");
            Console.WriteLine("------------------\n");
            //Console.WriteLine(flagsHeader);
